...
module.exports = async (args, options, logger) => {
  ...
  const afterName = async function(name) {
    ...
    // start to ask
    const resultYN = await questionService.askConfirm("Do you want to add an attiribute to " + name + " model?");
    if (!resultYN.value) {
      createModel(name);
    } else {
      // ASK ATTRIBUTES
      ...
      let askAttribute = async function() {
        ...
        if (resultType.value == "Relation") {
          ...
          const resultYN = await questionService.askConfirm("Do you want to add another attiribute to " + name + " model?");
          if (resultYN.value) {
            askAttribute();
          } else {
