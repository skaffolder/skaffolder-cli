...
var _findService = (yamlPaths, service_id) => {
			...
			if (_rel["x-skaffolder-ent1"] == model["x-skaffolder-id-entity"]) {
				...
				if (rel_model2) {
					...
					if (!_service) {
						...
						var obj = _createService(yamlProject, _service, { name: rel_model2["x-skaffolder-name"], url: rel_model2["x-skaffolder-url"] });
				...
				if (rel_model1) {
					...
					if (!_service) {
						...
						var obj = _createService(yamlProject, _service, { name: rel_model1["x-skaffolder-name"], url: rel_model1["x-skaffolder-url"] });
						yamlProject = obj.yamlProject;
						_service = obj._service;
					}
...
var createApiYAml = (service_yaml, service_method, _res) => {
	...
	if (yamlPaths) {
		...
		var service = offlineService.cloneObject(_findService(yamlPaths, service_yaml["x-skaffolder-id"]));
		var url = `${service_yaml["x-skaffolder-url"]}`.startsWith("/") ? service_yaml["x-skaffolder-url"] : `/${service_yaml["x-skaffolder-url"]}`
		var complete_url = `${_res.url}${url}`;
		service_method = `${service_method}`.toLowerCase();

		if (!service) {
			service = {
				"x-skaffolder-id": offlineService.getDummyId(`${service_yaml["x-skaffolder-name"] || service_method}_${_res.name}`, "service"),
			}
		...
		if (service_yaml.parameters) {
			service.parameters = service_yaml.parameters.map((val) => {
				...
				switch (val["x-skaffolder-type"]) {
					case "Date":
					case "Integer": val.schema.type = "integer"; break;
					case "Decimal":
					case "Number": val.schema.type = "number"; break;
					case "ObjectId":
					case "String": val.schema.type = "string"; break;
					case "Boolean": val.schema.type = "boolean"; break;
					case "Custom": val.schema.type = "object"; break;
					default: val.schema.type = val["x-skaffolder-type"];
				}
...
var createModelSkaffolder = (model_name, db_id, attributes, relations) => {
	var yamlProject = _getYamlProject();
	var model = {}
