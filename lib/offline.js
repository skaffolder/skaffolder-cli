...
const _createService = (yamlProject, service, _res) => {
  ...
  if (paths) {
    ...
    let serviceFullName = `${_res.url}${url}`;
  ...
  if (_relations) {
    for (id in _relations) {
      ...
      if (_rel["x-skaffolder-ent1"] == model["x-skaffolder-id-entity"]) {
        ...
        if (rel_model2) {
          let _service = _findServiceForResAndCrudAction(yamlProject.paths, rel_model2["x-skaffolder-id"], "list");
          if (!_service) {
            _service = {
        ...
        if (rel_model1) {
          ...
          let _service = _findServiceForResAndCrudAction(yamlProject.paths, rel_model1["x-skaffolder-id"], service_name);
