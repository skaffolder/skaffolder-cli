...
function writeGeneratorFiles(workspacePath, files) {
  files.filter(file => {
    ...
    if (file.templateList) {
      fs.writeFileSync(
        workspacePath + path + "_SK_LIST.hbs",
        file.templateList
      );
    ...
    if (file.templateEdit) {
      fs.writeFileSync(
        workspacePath + path + "_SK_EDIT.hbs",
        file.templateEdit
      );
...
function createProjectExtension(
  workspacePath,
  projectId,
  logger,
  frontendId,
  ...
  logger.info(
    chalk.blue("You can edit your project structure at ") +
      chalk.yellow(
        properties.endpoint + "/#!/projects/" + projectId + "/models"
      )
  );

  try {
    // CREATE TEMPLATE
    projectService.getTemplateFiles(
      frontendId.context,
      backendId.context,
